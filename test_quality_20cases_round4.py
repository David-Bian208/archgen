#!/usr/bin/env python3
"""
V4.5.4 20 案例质量验证测试 - 第 4 轮（工作流优化验证）
测试目的：验证 P0 工作流优化效果，目标报告生成率≥90%
"""
...
OUTPUT_DIR = Path("/home/admin/Desktop/V4.5.4-20 案例测试 - 第 4 轮")
OUTPUT_DIR.mkdir(parents=True, exist_ok=True)

# 20 个测试案例（与第 3 轮相同，便于对比）
TEST_CASES = [
    # === 提示依赖 (5 例) ===
...
def main():
    print("="*60)
    print("🧪 V4.5.4 20 案例质量验证测试 - 第 4 轮（工作流优化验证）")
    print(f"测试时间：{datetime.now().strftime('%Y-%m-%d %H:%M')}")
    print(f"测试重点：报告生成率（目标≥90%）")
    print("="*60)
    ...
    summary = {
        "test_info": {
            "version": "V4.5.4-Round4",
            "test_type": "20 案例质量验证 - 第 4 轮（工作流优化验证）",
            "total_cases": total,
            "passed": passed,
    ...
    import shutil
    desktop_md = Path("/home/admin/Desktop/V4.5.4-20 案例测试 - 第 4 轮报告.md")
...
def generate_markdown_report(summary):
    """生成 Markdown 报告"""
    md = f"""# V4.5.4 20 案例质量验证测试报告 - 第 4 轮
...
| **平均对话轮次** | 2.8 | {summary['test_info']['avg_turns']} | {'⬇️' if float(summary['test_info']['avg_turns']) < 2.8 else '➡️' if float(summary['test_info']['avg_turns']) == 2.8 else '⬆️'} | ≤6 | {'✅' if float(summary['test_info']['avg_turns']) <= 6 else '⚠️'} |
...
| 案例 | 第 3 轮 | 第 4 轮 | 状态 |
|------|--------|--------|------|
| 案例 2: 需要示范才做 | ❌ 无报告 | {'✅ 有报告' if summary['results'][1].get('passed') else '❌ 无报告'} | {'✅ 已修复' if summary['results'][1].get('passed') else '⚠️ 待修复'} |
| 案例 5: 穿衣看镜子 | ❌ 无报告 | {'✅ 有报告' if summary['results'][4].get('passed') else '❌ 无报告'} | {'✅ 已修复' if summary['results'][4].get('passed') else '⚠️ 待修复'} |
| 案例 12: 客人来捣乱 | ❌ 无报告 | {'✅ 有报告' if summary['results'][11].get('passed') else '❌ 无报告'} | {'✅ 已修复' if summary['results'][11].get('passed') else '⚠️ 待修复'} |
| 案例 14: 吃饭求关注 | ❌ 无报告 | {'✅ 有报告' if summary['results'][13].get('passed') else '❌ 无报告'} | {'✅ 已修复' if summary['results'][13].get('passed') else '⚠️ 待修复'} |
| 案例 19: 拒绝光脚 | ❌ 无报告 | {'✅ 有报告' if summary['results'][18].get('passed') else '❌ 无报告'} | {'✅ 已修复' if summary['results'][18].get('passed') else '⚠️ 待修复'} |

### 修复内容

1. **降低必填字段完成度阈值**: 100% → 75%
2. **增加早期退出条件**: 75% 完成度 + ≥3 轮即可结束
3. **避免重复提问**: 跟踪已问字段，跳过已问字段
...
| 问题 | 第 3 轮 | 第 4 轮 | 状态 |
|------|--------|--------|------|
| 报告生成率 | 80% | {summary['test_info']['pass_rate']} | {'✅ 已达标' if float(summary['test_info']['pass_rate'][:-1]) >= 90 else '⚠️ 待优化'} |
| 重复提问 | 常见 | {'✅ 无' if summary['test_info']['avg_turns'] <= 3.0 else '⚠️ 仍有'} | {'✅ 已修复' if summary['test_info']['avg_turns'] <= 3.0 else '⚠️ 待修复'} |
| 对话卡死 | 5/20 | {20 - summary['test_info']['passed']}/20 | {'✅ 已修复' if summary['test_info']['passed'] >= 18 else '⚠️ 待修复'} |

### 功能判断准确率对比

| 功能类型 | 第 3 轮 | 第 4 轮 | 变化 |
|---------|--------|--------|------|
| 提示依赖 | 60% (3/5) | {summary['category_stats'].get('提示依赖', {}).get('function_match', 0)}/{summary['category_stats'].get('提示依赖', {}).get('total', 1)} ({summary['category_stats'].get('提示依赖', {}).get('function_match', 0)/summary['category_stats'].get('提示依赖', {}).get('total', 1)*100:.0f}%) | {'⬆️ 提升' if summary['category_stats'].get('提示依赖', {}).get('function_match', 0) > 3 else '➡️ 持平' if summary['category_stats'].get('提示依赖', {}).get('function_match', 0) == 3 else '⬇️ 下降'} |
| 逃避难度 | 100% (5/5) | {summary['category_stats'].get('逃避难度', {}).get('function_match', 0)}/{summary['category_stats'].get('逃避难度', {}).get('total', 1)} ({summary['category_stats'].get('逃避难度', {}).get('function_match', 0)/summary['category_stats'].get('逃避难度', {}).get('total', 1)*100:.0f}%) | {'⬆️ 提升' if summary['category_stats'].get('逃避难度', {}).get('function_match', 0) > 5 else '➡️ 持平' if summary['category_stats'].get('逃避难度', {}).get('function_match', 0) == 5 else '⬇️ 下降'} |
...
| 感觉逃避 | 80% (4/5) | {summary['category_stats'].get('感觉逃避', {}).get('function_match', 0)}/{summary['category_stats'].get('感觉逃避', {}).get('total', 1)} ({summary['category_stats'].get('感觉逃避', {}).get('function_match', 0)/summary['category_stats'].get('感觉逃避', {}).get('total', 1)*100:.0f}%) | {'⬆️ 提升' if summary['category_stats'].get('感觉逃避', {}).get('function_match', 0) > 4 else '➡️ 持平' if summary['category_stats'].get('感觉逃避', {}).get('function_match', 0) == 4 else '⬇️ 下降'} |

---

**报告生成时间**: {datetime.now().strftime('%Y-%m-%d %H:%M')}  
**版本**: V4.5.4-Round4  
**状态**: {'✅ 测试通过，P0 已修复' if float(summary['test_info']['pass_rate'][:-1]) >= 90 else '⚠️ 仍需优化'}
"""
