...
【临床鉴别思考要求】
在生成报告时，需要简要说明鉴别诊断过程：针对观察到的行为，临床上通常会考虑几种可能性（如提示依赖、自我刺激、感觉逃避等），然后说明为什么结合当前信息判断某种可能性最高。
...
请严格按照以下 JSON 格式输出：
{
    ...
    "clinical_differential": "针对观察到的行为，说明临床上通常考虑的几种可能性（如：1) 提示依赖；2) 自我刺激；3) 感觉逃避），然后结合当前信息（如'有提示时表现良好'、'环境嘈杂'等）说明为什么某种可能性最高。100-200 字。",
