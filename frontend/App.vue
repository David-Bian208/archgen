...
export default {
  name: 'App',
  mounted() {
    console.log('[App] 📘 行为观察助手 V6.1.0')
    console.log('[App] 5 步临床推理引擎已就绪')
    console.log('[App] API 端点：/api/v6/analyze/stream')
    console.log('[App] 构建时间：' + new Date().toISOString())
  },
  data() {
    return {
      activeTab: 'chat',
  ...
  methods: {
    async sendMessage() {
      ...
      // V6.1.0 新增：版本追踪
      console.log('[App] V6.1.0 版本初始化完成')
      console.log('[App] 后端 API 地址：http://localhost:8001')
      console.log('[App] 前端运行端口：', window.location.port || '5173')
      ...
      try {
        ...
        const response = await axios.post('http://localhost:8001/api/v6/analyze', {
        ...
        // V6.1 格式适配：优先处理 V6.1 响应
        if (data.status === 'success' && data.data) {
          console.log('[App] 收到 V6.1 响应')
          const v6Data = data.data
          
          this.sessionCompleted = true
          
          // 提取 V6.1 数据
          this.insightReport = v6Data.report || {}
          this.interventionPlan = v6Data.intervention_plan || {}
          this.reportData = {
            context: v6Data.report?.context || '',
            child_behavior: v6Data.report?.child_behavior || '',
            others_response: v6Data.report?.others_response || '',
            scene: v6Data.report?.context || '',
          }
          
          // V6.1 新增：5 步推理数据
          if (v6Data.reasoning_steps) {
            this.reasoningSteps = v6Data.reasoning_steps.map((step, index) => ({
        ...
        // 处理旧版格式
        } else if (data.report && data.intervention_plan) {
          // 直接返回报告的格式（会话已完成）
          console.log('[App] 收到旧版完成报告')
          this.sessionCompleted = true
...
赋能思考：${this.insightReport.empowerment_question || ''}
    ...
    async submitFeedback() {
      ...
      try {
        const response = await axios.post('http://localhost:8001/api/v6/feedback', {
          session_id: this.sessionId,
