#!/usr/bin/env python3
"""
V4.5.3 20 案例质量验证测试 - 第 2 轮
测试目的：验证 P2 修复效果，测试功能判断和干预计划质量
"""
...
OUTPUT_DIR = Path("/home/admin/Desktop/V4.5.3-20 案例测试 - 第 2 轮")
OUTPUT_DIR.mkdir(parents=True, exist_ok=True)

# 20 个测试案例（4 大功能类型，每类 5 例）
TEST_CASES = [
    # === 提示依赖 (5 例) ===
...
def run_case(case):
    ...
    print(f"\n📊 结果：轮次={turn_count}, 报告={'有' if report else '无'}, 功能匹配={'✅' if function_match else '❌'}, 输出净化={'✅' if intervention_clean else '❌'}")
...
def main():
    print("="*60)
    print("🧪 V4.5.3 20 案例质量验证测试 - 第 2 轮")
    print(f"测试时间：{datetime.now().strftime('%Y-%m-%d %H:%M')}")
    print(f"测试重点：功能判断准确性 + 输出文本净化")
    print("="*60)
    ...
    both_good = sum(1 for r in results if r.get("function_match") and r.get("intervention_clean"))
    ...
    for r in results:
        ...
        if cat not in category_stats:
            category_stats[cat] = {"total": 0, "passed": 0, "function_match": 0}
    ...
    summary = {
        "test_info": {
            "version": "V4.5.3-Round2",
            "test_type": "20 案例质量验证 - 第 2 轮",
            "total_cases": total,
            "passed": passed,
    ...
    print(f"双达标（功能 + 净化）: {both_good}/{total}")
    print(f"平均轮次：{avg_turns:.1f}")
    print(f"\n📄 详情：{md_file}")
    
    # 复制到桌面
    import shutil
    desktop_md = Path("/home/admin/Desktop/V4.5.3-20 案例测试 - 第 2 轮报告.md")
...
def generate_markdown_report(summary):
    """生成 Markdown 报告"""
    md = f"""# V4.5.3 20 案例质量验证测试报告 - 第 2 轮

**测试时间**: {summary['test_info']['timestamp']}  
**测试版本**: V4.5.3-Round2  
**测试类型**: 质量验证（功能判断 + 输出文本净化）  
...
| **输出文本净化** | {summary['test_info']['intervention_clean']}/{summary['test_info']['total_cases']} | 20/20 | {'✅' if summary['test_info']['intervention_clean'] == 20 else '⚠️'} |
| **双达标** | {summary['test_info']['both_good']}/{summary['test_info']['total_cases']} | ≥18/20 | {'✅' if summary['test_info']['both_good'] >= 18 else '⚠️'} |
...
"""
    ...
    for cat, stats in summary['category_stats'].items():
        ...
        md += f"| {cat} | {total} | {passed}/{total} | {func}/{total} | {rate:.0f}% |\n"
...
| 输出文本净化 | 100% | {summary['test_info']['intervention_clean']}/{summary['test_info']['total_cases']} ({summary['test_info']['intervention_clean']/summary['test_info']['total_cases']*100:.1f}%) | {'✅' if summary['test_info']['intervention_clean'] == 20 else '⚠️'} |
| 双达标率 | ≥90% | {summary['test_info']['both_good']}/{summary['test_info']['total_cases']} ({summary['test_info']['both_good']/summary['test_info']['total_cases']*100:.1f}%) | {'✅' if summary['test_info']['both_good'] >= 18 else '⚠️'} |
...
"""
    ...
    for r in summary['results']:
        ...
        both_icon = "✅" if (r.get('function_match', False) and r.get('intervention_clean', False)) else "❌"
...
| 问题 | 修复前 | 修复后 | 状态 |
|------|--------|--------|------|
| 泄露置信度 | ❌ 常见 | {'✅ 无' if summary['test_info']['intervention_clean'] == 20 else '⚠️ 仍有'} | {'✅ 已修复' if summary['test_info']['intervention_clean'] == 20 else '⚠️ 待修复'} |
| 泄露证据条数 | ❌ 常见 | {'✅ 无' if summary['test_info']['intervention_clean'] == 20 else '⚠️ 仍有'} | {'✅ 已修复' if summary['test_info']['intervention_clean'] == 20 else '⚠️ 待修复'} |
| 语言简洁性 | ❌ 冗长 | {'✅ 简洁' if summary['test_info']['intervention_clean'] == 20 else '⚠️ 一般'} | {'✅ 已修复' if summary['test_info']['intervention_clean'] == 20 else '⚠️ 待修复'} |

### 功能判断准确率

| 功能类型 | 准确率 | 第 1 轮 | 变化 |
|---------|--------|--------|------|
| 提示依赖 | {summary['category_stats'].get('提示依赖', {}).get('function_match', 0)}/{summary['category_stats'].get('提示依赖', {}).get('total', 1)} | 3/5 (60%) | {'⬆️ 提升' if summary['category_stats'].get('提示依赖', {}).get('function_match', 0) > 3 else '➡️ 持平' if summary['category_stats'].get('提示依赖', {}).get('function_match', 0) == 3 else '⬇️ 下降'} |
| 逃避难度 | {summary['category_stats'].get('逃避难度', {}).get('function_match', 0)}/{summary['category_stats'].get('逃避难度', {}).get('total', 1)} | 5/5 (100%) | {'⬆️ 提升' if summary['category_stats'].get('逃避难度', {}).get('function_match', 0) > 5 else '➡️ 持平' if summary['category_stats'].get('逃避难度', {}).get('function_match', 0) == 5 else '⬇️ 下降'} |
| 寻求关注 | {summary['category_stats'].get('寻求关注', {}).get('function_match', 0)}/{summary['category_stats'].get('寻求关注', {}).get('total', 1)} | 2/5 (40%) | {'⬆️ 提升' if summary['category_stats'].get('寻求关注', {}).get('function_match', 0) > 2 else '➡️ 持平' if summary['category_stats'].get('寻求关注', {}).get('function_match', 0) == 2 else '⬇️ 下降'} |
| 感觉逃避 | {summary['category_stats'].get('感觉逃避', {}).get('function_match', 0)}/{summary['category_stats'].get('感觉逃避', {}).get('total', 1)} | 1/5 (20%) | {'⬆️ 提升' if summary['category_stats'].get('感觉逃避', {}).get('function_match', 0) > 1 else '➡️ 持平' if summary['category_stats'].get('感觉逃避', {}).get('function_match', 0) == 1 else '⬇️ 下降'} |

---

**报告生成时间**: {datetime.now().strftime('%Y-%m-%d %H:%M')}  
**版本**: V4.5.3-Round2  
**状态**: {'✅ 测试通过，P2 已修复' if summary['test_info']['intervention_clean'] == 20 else '⚠️ 仍需优化'}
"""
