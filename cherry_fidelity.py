...
THEMES = {
    ...
    "mono": {
        ...
        "primary": "#1F2937",
        "secondary": "#6B7280",
        "primary_10": "rgba(31,41,55,0.08)",
        "primary_5": "rgba(31,41,55,0.04)",
        "secondary_10": "rgba(107,114,128,0.08)",
        "secondary_5": "rgba(107,114,128,0.04)",
        "primary_60": "rgba(31,41,55,0.5)",
        "text_700": "#374151",
        "text_800": "#111827",
        "text_600": "#6B7280",
        "border_100": "#E5E7EB",
    ...
    "bake": {
        ...
        "primary": "#D97706",
        "secondary": "#F59E0B",
        "primary_10": "rgba(217,119,6,0.1)",
        "primary_5": "rgba(217,119,6,0.05)",
        "secondary_10": "rgba(245,158,11,0.1)",
        "secondary_5": "rgba(245,158,11,0.05)",
        "primary_60": "rgba(217,119,6,0.5)",
        "text_700": "#7C2D12",
        "text_800": "#431407",
        "text_600": "#92400E",
        "border_100": "#FDE68A",
        "bg_50": "#FFFDF7",
        "white": "#FFFDF7",
        "shadow": "0 1px 6px rgba(217,119,6,0.08)",
    },
}
...
def _esc(text: str) -> str:
    """HTML 实体转义，防止用户内容中的 <script>、<div> 等破坏 HTML 结构。"""
    if not text:
        return ""
    return (text
        .replace("&", "&amp;")
...
def _strip_bold_prefix(bold: str, text: str) -> str:
    """如果 text 开头已包含 bold: 或 bold： 前缀，则去除，防止重复渲染。"""
...
def _module_highlight(data: dict, num: str) -> str:
    """Module 01: 特殊高亮卡片 — 渐变背景 + 两列布局 + 右侧徽章（自适应比例）"""
    title = _esc(data.get("title", ""))
    items = data.get("items", [])
    badge_text = _esc(data.get("badge", ""))
    ...
    # 左侧: 要点列表，用 flex:1 自然撑满
    left_items = ""
    for item in items:
        icon_name = item.get("icon", "star")
        bold = _esc(item.get("bold", ""))
        text_content = _esc(item.get("text", ""))
        text_content = _strip_bold_prefix(bold, text_content)
        left_items += (
            ...
            f'line-height:1.25;">'
            + (f'<b>{bold}</b>：{text_content}' if bold else f'{text_content}')
            + f'</span></div>'
        )

    left_col = (
        f'<div style="display:flex;flex-direction:column;gap:{SPACE["highlight_item_gap"]}px;'
        f'flex:1;min-width:0;">{left_items}</div>'
    )

    # 右侧: 徽章（自适应宽度，横排）
    right_col = ""
    if badge_text:
        bw = _badge_width(badge_text)
        badge_font_size = FONT["badge_text"]
        
        right_col = (
            f'<div style="width:{bw}px;background:linear-gradient(180deg,{CHERRY["primary"]},{CHERRY["secondary"]});'
            ...
            f'padding:{SPACE["badge_pad"]}px;flex-shrink:0;'
            f'font-size:{badge_font_size}px;font-weight:700;line-height:1.4;">'
            f'{badge_text}'
            f'</div>'
        )
...
def _module_list_card(data: dict, num: str, half_width: bool = False) -> str:
    """Module 02/03/04/06: 标准白色卡片 — 标题行 + 列表/网格内容 + 可选浮标"""
    title = _esc(data.get("title", ""))
    items = data.get("items", [])
    is_grid = data.get("layout") == "grid"  # 3-column grid for modules 4, 6
    badge_text = _esc(data.get("badge", ""))
    badge_mode = data.get("badge_mode", "highlight")
    # 浮标模式：用户明确选 float 才显示，否则默认 highlight 大徽章
    # 半宽卡片不显示浮标（空间太小）
    show_float_badge = bool(badge_text) and badge_mode == "float" and not half_width
    ...
    if is_grid:
        ...
        for item in items:
            icon_name = item.get("icon", "star")
            cell_title = _esc(item.get("bold", ""))
            cell_desc = _esc(item.get("text", ""))
            cell_desc = _strip_bold_prefix(cell_title, cell_desc)  # 去重前缀
            cells += (
        ...
        # 列表模式 (module 02/03) — 支持段落浮标
        list_items = ""
        for item in items:
            icon_name = item.get("icon", "star")
            bold = _esc(item.get("bold", ""))
            text = _esc(item.get("text", ""))
            text = _strip_bold_prefix(bold, text)  # 去重前缀，避免 "四方对齐：四方对齐：xxx"
            list_items += (
                f'<div style="display:flex;align-items:center;gap:{SPACE["title_gap"]}px;">'
                ...
                f'<span style="font-size:{body_font}px;color:{text_color};line-height:1.25;">'
        ...
        if show_float_badge:
            bw = _badge_width(badge_text)
            # 段落浮标比全文核心徽章小一号
            small_bw = max(60, bw // 2)
            float_badge_html = (
                f'<div style="position:absolute;right:{SPACE["module_pad"]}px;top:{SPACE["module_pad"]}px;'
                f'width:{small_bw}px;'
                f'background:linear-gradient(180deg,{CHERRY["primary"]},{CHERRY["secondary"]});'
                f'border-radius:{SPACE["cell_radius"]}px;'
                f'color:{CHERRY["white"]};text-align:center;'
                f'padding:{round(SPACE["badge_pad"]/1.5)}px;'
                f'font-size:{FONT["body_small"]}px;font-weight:700;line-height:1.3;'
                f'z-index:1;">{badge_text}</div>'
            )
            content_padding_right = f'padding-right:{small_bw + SPACE["module_gap"]}px;'

        content = (
            f'<div style="position:relative;display:flex;flex-direction:column;'
            f'gap:{SPACE["module_gap"]}px;{content_padding_right}">{float_badge_html}{list_items}</div>'
        )
...
def _smart_flow_break(text: str, max_chars: int = 12) -> str:
    """
    flow 节点文字智能断行。
    优先 LLM 标注的 | 断点，否则按语义边界自动断。
    绝不切断英文单词。
    """
    if not text:
        return text

    # 规则0：LLM 标注了 | 断点 → 直接插 <br>
    if '|' in text:
        parts = [p.strip() for p in text.split('|')]
        return '<br>'.join(parts)

    # 规则1：有空格 → 在空格处断（英文+中文的天然断点）
    if ' ' in text:
        # 找最中间的断点
    ...
    # 规则2：有全角标点 → 在标点后断
    puncts = [m.start() for m in re.finditer(r'[，。、；：！？]', text)]
    if puncts:
        mid = len(text) // 2
        best = min(puncts, key=lambda x: abs(x - mid))
        left = text[:best + 1]
        right = text[best + 1:]
        if len(left) >= 3 and len(right) >= 3:
            return f'{left}<br>{right}'

    # 规则3：中英交界处 → 在交界处断（不在数字内部断）
    boundaries = []
    for i in range(1, len(text)):
        a, b = text[i - 1], text[i]
        # 中<->英边界
        if ('\u4e00' <= a <= '\u9fff' and b.isascii() and b.isalpha()):
            boundaries.append(i)
        elif (a.isascii() and a.isalpha() and '\u4e00' <= b <= '\u9fff'):
            boundaries.append(i)
        # / - & _ 分隔符（前后不都是数字时）
        elif a in '/&-_' and b != ' ':
            prev_char = text[i - 2] if i >= 2 else ''
            if not (prev_char.isdigit() and b.isdigit()):
                boundaries.append(i)
    if boundaries:
        mid = len(text) // 2
        best = min(boundaries, key=lambda x: abs(x - mid))
        left = text[:best]
        right = text[best:]
        if len(left) >= 3 and len(right) >= 3:
            return f'{left}<br>{right}'
    ...
    # 规则5：纯中文 → 均分两行，不切英文词
    is_cjk = all('\u4e00' <= c <= '\u9fff' for c in text)
    if is_cjk:
        mid = (len(text) + 1) // 2
        ...
        for direction in [1, -1]:
            ...
            if 3 <= pos <= len(text) - 3:
                ...
                if a.isdigit() and b in ':-':
                    continue  # 在数字格式中间（如 8:00、200-300）
...
# ============================================================
# flow 节点图标关键词映射（渲染层，不改 LLM prompt）
# ============================================================

_FLOW_ICON_MAP = [
    # (关键词, 图标名) — 按顺序匹配，先匹配到的生效
    ("采集", "download"), ("拉取", "download"), ("抓取", "download"),
    ("清洗", "filter"), ("去重", "filter"), ("过滤", "filter"), ("筛选", "filter"),
    ("评分", "star"), ("排序", "chart"), ("排名", "chart"),
    ("分发", "rocket"), ("推送", "rocket"), ("发布", "rocket"),
    ("分析", "search"), ("检索", "search"),
    ("处理", "code"), ("计算", "code"), ("编码", "code"), ("任务", "code"),
    ("存储", "database"), ("入库", "database"), ("保存", "database"),
    ("定时", "clock"), ("调度", "clock"), ("触发", "clock"),
    ("生成", "file"), ("输出", "file"), ("导出", "file"),
    ("检查", "check"), ("校验", "check"), ("验证", "check"),
    ("部署", "rocket"), ("上线", "rocket"),
    ("同步", "layers"), ("对接", "network"),
    ("对齐", "target"), ("匹配", "target"),
]


def _flow_item_icon(bold: str, text: str) -> str:
    """根据 flow 节点的 bold/text 内容匹配图标名。"""
    combined = ((bold or "") + (text or "")).lower()
    for keyword, icon in _FLOW_ICON_MAP:
...
def _module_flow(data: dict, num: str) -> str:
    ...
    for i, item in enumerate(items):
        icon_name = _flow_item_icon(item.get("bold") or "", item.get("text") or "")
        text = _smart_flow_break(_esc(item.get("text", "")))
        nodes += (
            f'<div style="display:flex;flex-direction:column;align-items:center;width:{round(100/n)}%;">'
            # 图标：固定高度，底部对齐（确保所有图标在同一水平线）
            f'<div style="height:{FONT["flow_icon"] + 8}px;display:flex;align-items:flex-end;justify-content:center;">'
            f'<span style="color:{CHERRY["primary"]};font-size:{FONT["flow_icon"]}px;line-height:1;">'
            f'{_icon(icon_name, FONT["flow_icon"])}</span></div>'
            # 文字：固定两行高度（40px），居中对齐，第二行没文字就空着
            f'<div style="margin-top:{round(SPACE["module_gap"]/3)}px;text-align:center;font-size:{FONT["body_flow"]}px;'
            f'color:{CHERRY["text_700"]};line-height:1.3;min-height:40px;display:flex;'
            f'align-items:flex-start;justify-content:center;width:100%;">{text}</div>'
            f'</div>'
        )
        if i < n - 1:
            nodes += (
                f'<span style="color:{CHERRY["primary_60"]};font-size:{FONT["arrow_icon"]}px;'
                f'display:flex;align-items:center;flex-shrink:0;padding-bottom:20px;">'
                f'{_icon("chevron-right", FONT["arrow_icon"])}</span>'
            )

    content = (
        f'<div style="display:flex;align-items:stretch;justify-content:space-between;'
...
def _module_compare(data: dict, num: str) -> str:
    """Module 06: 对比卡片 — 左右两栏 + 中间 VS 分割线"""
    title = _esc(data.get("title", ""))
    items = data.get("items", [])
    n = len(items)
    left_n = (n + 1) // 2
    ...
    def _render_items(item_list):
        ...
        for item in item_list:
            ...
            bold = _esc(item.get("bold", ""))
            text = _esc(item.get("text", ""))
            text = _strip_bold_prefix(bold, text)
            html += (
                f'<div style="display:flex;align-items:flex-start;gap:{SPACE["content_gap"]}px;">'
                f'<span style="color:{CHERRY["primary"]};width:{SPACE["icon_half"]}px;'
                f'text-align:center;flex-shrink:0;margin-top:2px;">'
                f'{_icon(icon_name, FONT["body_list"])}</span>'
    ...
    left_panel = (
        f'<div style="flex:1;background:{CHERRY["bg_50"]};'
        f'border-radius:{SPACE["cell_radius"]}px;'
        f'padding:{SPACE["grid_cell_pad"]}px;display:flex;flex-direction:column;'
        f'gap:{SPACE["item_gap"]}px;">{_render_items(left_items)}</div>'
    )
    ...
    right_panel = (
        ...
        f'border-radius:{SPACE["cell_radius"]}px;'
        f'padding:{SPACE["grid_cell_pad"]}px;display:flex;flex-direction:column;'
        f'gap:{SPACE["item_gap"]}px;">{_render_items(right_items)}</div>'
    )
    ...
    divider = (
        ...
        f'justify-content:center;flex-shrink:0;gap:{round(SPACE["module_gap"]/3)}px;">'
        f'<div style="width:1px;flex:1;background:{CHERRY["border_100"]};"></div>'
        f'<div style="width:{vs_size}px;height:{vs_size}px;'
        f'background:linear-gradient(135deg,{CHERRY["primary"]},{CHERRY["secondary"]});'
        f'border-radius:50%;color:{CHERRY["white"]};'
        f'font-size:{FONT["body_small"]}px;font-weight:700;'
    ...
    content = (
        f'<div style="display:flex;gap:{round(SPACE["module_gap"]/2)}px;'
        f'align-items:stretch;">{left_panel}{divider}{right_panel}</div>'
    )
...
def render_cherry_fidelity(
    ...
    footer_html = (
        ...
        f'padding:0 {module_gap}px;flex-shrink:0;">'
        f'<p style="font-size:{FONT["footer_text"]}px;color:{CHERRY["white"]};margin:0;">{_esc(footer_text)}</p></div>'
    )
...
(function(){{
    ...
    // 自适应高度开关：URL 参数优先，其次是 window 注入（截图场景）
    var autoHeight=false;
    try{{
        var urlParams=new URLSearchParams(window.location.search);
        autoHeight=urlParams.get('auto_height')==='1';
    }}catch(e){{}}
    if(!autoHeight && window.forceAutoHeight){{
        autoHeight=true;
    }}
    ...
    if(autoHeight){{
        ...
        if(poster){{
            var headerH=poster.firstElementChild?poster.firstElementChild.offsetHeight:58;
            var footerH=poster.lastElementChild?poster.lastElementChild.offsetHeight:58;
            var newTotal=headerH+totalH+footerH+28;
            poster.style.height=newTotal+'px';
        }}
    }}else if(totalH>maxH){{
        // 内容溢出 → 压缩
        ...
        // 内容不足 → 方案E放大（强化版，上限 2.0）
        var gap=maxH-totalH;
        if(gap<100){{
            var items=c.firstElementChild.children;
            if(items.length>1){{
                var extra=Math.floor(gap/(items.length-1));
                c.firstElementChild.style.gap=({module_gap}+extra)+'px';
            }}
        }}else{{
            var s=Math.min(maxH/totalH,2.0);
            if(s>1.0){{
                c.style.transform='scaleY('+s+')';
                c.style.transformOrigin='top center';
                c.firstElementChild.style.gap={module_gap}+'px';
            }}
        }}
    }}
}})();
...
def _card_to_module(card: dict, num: str, half_width: bool = False) -> dict:
    ...
    badge_mode = card.get("badge_mode", "highlight")  # "highlight" | "float"
    items = card.get("items", [])
    n_items = len(items)

    # 确定渲染类型（shape 类优先，它们定义了内容的视觉结构）
    if shape == "flow":
        mtype = "flow"
    elif shape == "grid":
        mtype = "grid"
        ...
        mtype = "compare"
    elif badge and badge_mode == "highlight":
        # highlight 模式：渐变背景 + 右侧大徽章（默认，视觉效果最佳）
        mtype = "highlight"
    ...
    print(f"[_card_to_module] 标题:{card.get('title','')[:20]}, shape:{shape}, density:{density}, badge:{bool(badge)}, mode:{badge_mode}, items:{n_items} → mtype:{mtype}")

    # 强制使用卡片级 icon（用户在编辑面板设置的优先级最高）
...
def render_from_rows(
    ...
    spacing: "adaptive"（智能撑开）| "default"（14px）| int（自定义 px）
    theme: "blue" | "mono" | "bake"
    """
    global CHERRY
    old_cherry = CHERRY
    CHERRY = THEMES.get(theme, THEMES["blue"])
    w = 1080 if canvas_type == "portrait" else 1920
    h = 1440 if canvas_type == "portrait" else 1080

    # 计算模块间距
    nr = len(rows)
    mr = 5 if canvas_type == "portrait" else 4
    if spacing == "adaptive":
        module_gap = max(4, min(SPACE["module_gap"] + (mr - nr) * 8, 40))
    elif isinstance(spacing, int):
        module_gap = spacing
    else:
        module_gap = SPACE["module_gap"]  # "default" = 14px
    ...
    for row in rows:
        ...
        if n_slots == 1:
            ...
            if mtype == "highlight":
                ...
                module_html_list.append(_module_compare(module, num_str))
            else:
                module_html_list.append(_module_list_card(module, num_str))
        else:
            # 多列行（拼车）
            ...
            row_html = (
                f'<div style="display:flex;gap:{module_gap}px;">'
                + "".join(cards_html)
                + "</div>"
            )
    ...
    # 模块间间距
    body_content = (
        f'<div style="display:flex;flex-direction:column;gap:{module_gap}px;">'
