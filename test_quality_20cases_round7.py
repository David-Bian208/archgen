#!/usr/bin/env python3
"""
V4.5.7 20 案例质量验证测试 - 第 7 轮（P0 修复验证）
测试目的：验证矛盾证据处理修复效果，目标寻求关注准确率≥80%
"""
...
OUTPUT_DIR = Path("/home/admin/Desktop/V4.5.7-20 案例测试 - 第 7 轮")
...
def main():
    print("="*60)
    print("🧪 V4.5.7 20 案例质量验证测试 - 第 7 轮（P0 修复验证）")
    print(f"测试时间：{datetime.now().strftime('%Y-%m-%d %H:%M')}")
    print(f"测试重点：矛盾证据处理（案例 13、14）")
    print("="*60)
    ...
    summary = {
        "test_info": {
            "version": "V4.5.7-Round7",
            "test_type": "20 案例质量验证 - 第 7 轮（P0 修复验证）",
            "total_cases": total,
            "passed": passed,
    ...
    import shutil
    desktop_md = Path("/home/admin/Desktop/V4.5.7-20 案例测试 - 第 7 轮报告.md")
...
def generate_markdown_report(summary):
    """生成 Markdown 报告"""
    md = f"""# V4.5.7 20 案例质量验证测试报告 - 第 7 轮
...
| **功能判断匹配** | 70% | {summary['test_info']['function_match']}/{summary['test_info']['total_cases']} ({summary['test_info']['function_match']/summary['test_info']['total_cases']*100:.0f}%) | {'⬆️' if summary['test_info']['function_match']/summary['test_info']['total_cases'] > 0.70 else '➡️' if summary['test_info']['function_match']/summary['test_info']['total_cases'] == 0.70 else '⬇️'} | ≥90% | {'✅' if summary['test_info']['function_match'] >= 18 else '⚠️'} |
...
| **三达标** | 70% | {summary['test_info']['all_good']}/{summary['test_info']['total_cases']} ({summary['test_info']['all_good']/summary['test_info']['total_cases']*100:.0f}%) | {'⬆️' if summary['test_info']['all_good']/summary['test_info']['total_cases'] > 0.70 else '➡️' if summary['test_info']['all_good']/summary['test_info']['total_cases'] == 0.70 else '⬇️'} | ≥90% | {'✅' if summary['test_info']['all_good'] >= 18 else '⚠️'} |
| **平均对话轮次** | 3.1 | {summary['test_info']['avg_turns']} | {'⬇️' if float(summary['test_info']['avg_turns']) < 3.1 else '➡️' if float(summary['test_info']['avg_turns']) == 3.1 else '⬆️'} | ≤6 | {'✅' if float(summary['test_info']['avg_turns']) <= 6 else '⚠️'} |
...
### 案例 13: 写作业求关注（P0 修复验证）

**测试输入**: "写作业时不停地叫我，其实他都会做。"

**第 6 轮**: ❌ 提示依赖下的逃避  
**第 7 轮**: {summary['results'][12].get('report', {}).get('report', {}).get('functional_judgment', '无报告') if summary['results'][12].get('report') else '无报告'}

**修复状态**: {'✅ 已修复' if summary['results'][12].get('function_match') else '❌ 未修复'}
...
| 寻求关注判断准确率低 | 60% | {summary['category_stats'].get('寻求关注', {}).get('function_match', 0)/summary['category_stats'].get('寻求关注', {}).get('total', 1)*100:.0f}% | {'✅ 已达标' if summary['category_stats'].get('寻求关注', {}).get('function_match', 0)/summary['category_stats'].get('寻求关注', {}).get('total', 1) >= 0.8 else '⚠️ 待优化'} |
...
| 判断准确性 | 3 | {'4' if summary['test_info']['function_match']/summary['test_info']['total_cases'] >= 0.85 else '3'} | {'⬆️ 提升' if summary['test_info']['function_match']/summary['test_info']['total_cases'] >= 0.85 else '➡️ 持平'} |
| 建议匹配度 | 5 | 5 | ➡️ 持平 |
| 沟通专业性 | 5 | 5 | ➡️ 持平 |
| 系统稳定性 | 3 | {'4' if float(summary['test_info']['pass_rate'][:-1]) >= 85 else '3'} | {'⬆️ 提升' if float(summary['test_info']['pass_rate'][:-1]) >= 85 else '➡️ 持平'} |
| 总体可靠性 | 3 | {'4' if summary['test_info']['function_match']/summary['test_info']['total_cases'] >= 0.85 and float(summary['test_info']['pass_rate'][:-1]) >= 85 else '3'} | {'⬆️ 提升' if summary['test_info']['function_match']/summary['test_info']['total_cases'] >= 0.85 and float(summary['test_info']['pass_rate'][:-1]) >= 85 else '➡️ 持平'} |

---

**报告生成时间**: {datetime.now().strftime('%Y-%m-%d %H:%M')}  
**版本**: V4.5.7-Round7  
**状态**: {'✅ 测试通过，P0 已修复' if summary['category_stats'].get('寻求关注', {}).get('function_match', 0)/summary['category_stats'].get('寻求关注', {}).get('total', 1) >= 0.8 else '⚠️ 部分改善，仍需优化'}
"""
