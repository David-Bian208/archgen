...
_agent_v4: GuidedRecorderAgentV4 | None = None


def get_agent_v4() -> GuidedRecorderAgentV4:
    """获取 V4.0 Agent 实例"""
    global _agent_v4
    ...
    if _agent_v4 is None:
        ...
        _agent_v4 = GuidedRecorderAgentV4(llm_client)
        logger.info("V4.0 Agent 实例已创建")
...
async def chat_v4(request: dict[str, Any]):
    ...
    try:
        ...
        logger.info(f"V4.0 响应：status={response['status']}")
        return response
