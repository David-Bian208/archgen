...
class LLMCache:
    ...
    def get(self, system_prompt: str, user_prompt: str) -> Optional[str]:
        ...
        try:
            ...
            # 检查是否过期（支持每个条目的自定义 TTL）
            entry_ttl = data.get('ttl', self.ttl_seconds)
            if time.time() - data['timestamp'] > entry_ttl:
    ...
    def set(self, system_prompt: str, user_prompt: str, response: str, ttl_seconds: Optional[int] = None) -> None:
        """
        设置缓存
        ...
        Args:
            ...
            user_prompt: 用户提示词
            response: LLM 响应文本
            ttl_seconds: 可选的自定义过期时间（秒），None 则使用默认值
        """
        cache_key = self._generate_cache_key(system_prompt, user_prompt)
        cache_file = self.cache_dir / f"{cache_key}.json"
