...
5. **语义判断优化**：A+C 方案（关键词扩展 + LLM 兜底）解决认知错误型行为识别问题
...
└───────────────────────────┬─────────────────────────────────┘
                            │
                    ┌───────▼───────┐
                    │  Chainlit UI  │  端口 8005 (V6.4)
                    │  (对话界面)    │  流式进度 + 折叠推理 + 报告
                    └───────┬───────┘
                            │
                    ┌───────▼───────┐
                    │  推理引擎     │  ClinicalReasoningEngine (V6.4)
                    │  (LLM 驱动)   │  4-5 次 LLM 调用（5 步推理 + LLM 兜底）
                    └───────┬───────┘
                            │
                    ┌───────▼───────┐
...
| `chainlit_v62/start_chainlit.sh` | **启动脚本** | 固定端口 8005，自动清理旧进程 |
| `app/agents/clinical_reasoning_engine.py` | **核心引擎** | V6.4 推理引擎，5 步临床推理 + ABC 提取 + 报告生成 + LLM 兜底 |
...
用户输入
  ...
  │   ├─ valid=False + vague → LLM 兜底判断
  │   │   ├─ LLM 认为有具体描述 → complete（直接推理）
  │   │   └─ LLM 认为仍模糊 → ABC 教育 + 引导
  │   ├─ valid=False + greeting/short → 直接拒绝
  │   ├─ quality=complete → 直接推理
  │   ├─ quality=partial + 输入>150 字 → 直接推理
  │   ├─ quality=partial + 输入≤150 字 → ABC 引导
  │   └─ quality=vague → LLM 兜底（见上）
  │
...
| LLM 兜底 | 关键词检测失败（vague）时触发 | 🔶 智能判断 |

### 2.2 关键词列表（V6.4 扩展版）

**BEHAVIOR_KEYWORDS（行为关键词）：**
...
### 2.3 LLM 兜底判断（V6.4 新增）

**文件：** `app/agents/clinical_reasoning_engine.py` — `_llm_fallback_check()`

**触发条件：** 仅当关键词检测为 `vague` 时触发

**判断逻辑：**
```python
prompt = f"""请判断以下用户输入是否包含**具体的行为场景描述**：

用户输入："{user_input}"

判断标准：
- 有具体：描述了具体的时间/地点/事件/行为
  （如"昨天在公园摔倒了"、"做作业时把 6 看成 9"）
- 无具体：只有情绪词/概括性评价/求助
  （如"孩子最近不好"、"总是发脾气"、"怎么办"）

请只输出 JSON 格式：
{{
    "has_concrete_description": true/false,
    "reason": "简短理由（20 字内）"
}}"""
```

**性能影响：**
- 约 90% 的输入（complete/partial）→ 0 额外调用
- 约 10% 的输入（vague）→ 增加 1 次 LLM 调用（1-3 秒，<150 tokens）

**异常处理：** LLM 调用失败时，保守处理为 `vague`，触发 ABC 教育。

### 2.4 ABC 引导式对话（V6.3 新增，V6.4 精简）
...
**性能：** 总耗时 ~15-20 秒，4-5 次 LLM 调用（vague 输入 +1 次兜底）
...
- 恢复"场景理解"（去掉类型代码如"A 类"）
- 免责声明精简
- **年龄推断说明**：如用户未提供年龄，LLM 基于行为推断典型年龄段，并说明是"典型表现年龄段"而非确诊
...
| **8005** | **Chainlit UI（V6.4）** | **运行中** | **当前主要入口** |
| 8002-8004 | 历史 chainlit 实例 | 已 kill | 沙箱环境占着释放不了，等待 TIME_WAIT 超时 |

**端口清理：** 沙箱容器环境导致的端口占用（TIME_WAIT）无法通过 `kill` 释放，不影响当前服务。启动脚本 `start_chainlit.sh` 会自动检测并清理端口。
...
用户打开 http://localhost:8005
  ...
  ├─ 用户输入行为描述
  │   │
  │   ├─ 完整输入（quality=complete）
  │   │   → 直接推理
  │   │   → 进度流式展示（🤔 Step 1~5）
  │   │   → 折叠的推理过程（灰色小字，可展开）
  │   │   → 正文报告（场景理解 → ABC → 行为假设 → 机制 → 安全 → 策略 → 免责声明）
  │   │
  │   ├─ 部分输入（quality=partial）
  │   │   ├─ 输入>150 字 → 直接推理
  │   │   └─ 输入≤150 字 → ABC 引导（精简话术）→ 收集完成后触发推理
  │   │
  │   ├─ 模糊输入（quality=vague）
  │   │   → LLM 兜底判断（1-3 秒）
  │   │   ├─ LLM 认为有具体 → 直接推理
...
### 6.1 认知错误型行为识别（方案 A+C）

**问题案例：**
> "上课的内容，玥玥有时候会把方向搞反，比如 6 和 9 会看错..."

**根因：** 关键词列表偏向"情绪爆发型"（哭、叫、发脾气），缺少"认知错误型"（看错、搞反、混淆）

**修复方案：**
1. **方案 A**：扩展 BEHAVIOR_KEYWORDS（认知错误型）+ SCENE_KEYWORDS（学习场景）
2. **方案 C**：LLM 兜底判断（仅 vague 时触发，<150 tokens，1-3 秒）

**效果：** 90% 输入 0 额外延迟，10% vague 输入增加 LLM 智能判断

### 6.2 年龄推断说明

**问题：** LLM 基于行为表现推断年龄段（如"4-6 岁"），但未说明是推断，用户可能误认为"确诊"

**修复：** Step 4 prompt 新增要求：
> "如用户未提供年龄，请基于行为表现推断典型年龄段，并说明这是'典型表现年龄段'而非确诊"

**示例输出：**
> 发展视角：基于描述的行为表现，这在**4-6 岁儿童中较为典型**（非确诊）。当前阶段 [前额叶皮层] 发育初期...
...
3. **沙箱端口占用**：8002-8004 被沙箱容器占着释放不了（TIME_WAIT），不影响当前服务
4. **推理过程折叠**：`<details>` 标签在浅色主题下背景色 `#fafafa` 可能不够明显，可根据实际效果调整
5. **LLM 兜底成本**：约 10% 的输入会增加 1 次 LLM 调用（<150 tokens），总体成本可控
...
| **V6.4.1** | **2026-04-24** | **关键词扩展（认知错误型）+ LLM 兜底判断 + 年龄推断说明** |
...
### 短期（已完成）
- ✅ 关键词扩展（认知错误型行为 + 学习场景）
- ✅ LLM 兜底判断（vague 输入智能判断）
- ✅ 年龄推断说明（避免误认为"确诊"）

### 中期（1-2 周）
- 监控 LLM 兜底触发率（目标<10%）
- 根据实际数据优化兜底 prompt
- 考虑用更轻量模型（如 DeepSeek-Coder）降低兜底成本

### 长期
- 考虑用语义嵌入（Embedding）替代关键词检测
- 建立用户反馈机制，持续优化判断准确性
