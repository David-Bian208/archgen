#!/usr/bin/env python3
"""
V4.5.3 20 案例质量验证测试 - 第 3 轮（关键问题修复验证）
测试目的：验证 P0 诊断 - 干预错配修复效果
"""
...
OUTPUT_DIR = Path("/home/admin/Desktop/V4.5.3-20 案例测试 - 第 3 轮")
OUTPUT_DIR.mkdir(parents=True, exist_ok=True)

# 20 个测试案例（重点关注感觉逃避和寻求关注）
TEST_CASES = [
    # === 提示依赖 (5 例) ===
...
def main():
    print("="*60)
    print("🧪 V4.5.3 20 案例质量验证测试 - 第 3 轮（关键问题修复验证）")
    print(f"测试时间：{datetime.now().strftime('%Y-%m-%d %H:%M')}")
    print(f"测试重点：诊断 - 干预匹配 + 功能判断准确率")
    print("="*60)
    ...
    summary = {
        "test_info": {
            "version": "V4.5.3-Round3",
            "test_type": "20 案例质量验证 - 第 3 轮（关键问题修复验证）",
            "total_cases": total,
            "passed": passed,
    ...
    print(f"三达标（功能 + 干预 + 净化）: {all_good}/{total}")
    print(f"平均轮次：{avg_turns:.1f}")
    print(f"\n📄 详情：{md_file}")
    
    # 复制到桌面
    import shutil
    desktop_md = Path("/home/admin/Desktop/V4.5.3-20 案例测试 - 第 3 轮报告.md")
...
def generate_markdown_report(summary):
    """生成 Markdown 报告"""
    md = f"""# V4.5.3 20 案例质量验证测试报告 - 第 3 轮

**测试时间**: {summary['test_info']['timestamp']}  
**测试版本**: V4.5.3-Round3  
**测试类型**: 关键问题修复验证（诊断 - 干预匹配）  
...
| 指标 | 第 2 轮 | 第 3 轮 | 变化 | 目标 | 状态 |
|------|--------|--------|------|------|------|
| **报告生成率** | 75% | {summary['test_info']['pass_rate']} | {'⬆️' if float(summary['test_info']['pass_rate'][:-1]) > 75 else '➡️' if float(summary['test_info']['pass_rate'][:-1]) == 75 else '⬇️'} | ≥90% | {'✅' if float(summary['test_info']['pass_rate'][:-1]) >= 90 else '⚠️'} |
| **功能判断匹配** | 65% | {summary['test_info']['function_match']}/{summary['test_info']['total_cases']} ({summary['test_info']['function_match']/summary['test_info']['total_cases']*100:.0f}%) | {'⬆️' if summary['test_info']['function_match']/summary['test_info']['total_cases'] > 0.65 else '➡️' if summary['test_info']['function_match']/summary['test_info']['total_cases'] == 0.65 else '⬇️'} | ≥90% | {'✅' if summary['test_info']['function_match'] >= 18 else '⚠️'} |
| **诊断 - 干预匹配** | N/A | {summary['test_info']['intervention_match']}/{summary['test_info']['total_cases']} ({summary['test_info']['intervention_match']/summary['test_info']['total_cases']*100:.0f}%) | - | 100% | {'✅' if summary['test_info']['intervention_match'] == 20 else '⚠️'} |
...
| **三达标** | 65% | {summary['test_info']['all_good']}/{summary['test_info']['total_cases']} ({summary['test_info']['all_good']/summary['test_info']['total_cases']*100:.0f}%) | {'⬆️' if summary['test_info']['all_good']/summary['test_info']['total_cases'] > 0.65 else '➡️' if summary['test_info']['all_good']/summary['test_info']['total_cases'] == 0.65 else '⬇️'} | ≥90% | {'✅' if summary['test_info']['all_good'] >= 18 else '⚠️'} |
| **平均对话轮次** | 3.0 | {summary['test_info']['avg_turns']} | {'➡️' if float(summary['test_info']['avg_turns']) == 3.0 else '⬆️' if float(summary['test_info']['avg_turns']) > 3.0 else '⬇️'} | ≤6 | {'✅' if float(summary['test_info']['avg_turns']) <= 6 else '⚠️'} |
...
**修复后**:
- 功能判断：{summary['results'][16].get('report', {}).get('report', {}).get('functional_judgment', 'N/A') if summary['results'][16].get('report') else '无报告'}
- 干预计划：{summary['results'][16].get('report', {}).get('intervention_plan', {}).get('phase_name', 'N/A') if summary['results'][16].get('report') else '无报告'}
- 诊断 - 干预匹配：{'✅' if summary['results'][16].get('intervention_match', False) else '❌'}

### 案例 13: 写作业求关注（寻求关注）

**测试输入**: "写作业时不停地叫我，其实他都会做。"
...
**修复后**:
- 功能判断：{summary['results'][12].get('report', {}).get('report', {}).get('functional_judgment', 'N/A') if summary['results'][12].get('report') else '无报告'}
- 干预计划：{summary['results'][12].get('report', {}).get('intervention_plan', {}).get('phase_name', 'N/A') if summary['results'][12].get('report') else '无报告'}
- 诊断 - 干预匹配：{'✅' if summary['results'][12].get('intervention_match', False) else '❌'}
...
| 三达标率 | ≥90% | {summary['test_info']['all_good']}/{summary['test_info']['total_cases']} ({summary['test_info']['all_good']/summary['test_info']['total_cases']*100:.0f}%) | {'✅' if summary['test_info']['all_good'] >= 18 else '⚠️'} |
...
| 问题 | 修复前 | 修复后 | 状态 |
|------|--------|--------|------|
| 感觉逃避错配到序列支持 | ❌ 常见 | {'✅ 无' if summary['test_info']['intervention_match'] == 20 else '⚠️ 仍有'} | {'✅ 已修复' if summary['test_info']['intervention_match'] == 20 else '⚠️ 待修复'} |
| 寻求关注错配到提示依赖 | ❌ 常见 | {'✅ 无' if summary['test_info']['intervention_match'] == 20 else '⚠️ 仍有'} | {'✅ 已修复' if summary['test_info']['intervention_match'] == 20 else '⚠️ 待修复'} |
| 输出文本泄露内部数据 | ❌ 常见 | {'✅ 无' if summary['test_info']['intervention_clean'] == 20 else '⚠️ 仍有'} | {'✅ 已修复' if summary['test_info']['intervention_clean'] == 20 else '⚠️ 待修复'} |

### 功能判断准确率对比

| 功能类型 | 第 2 轮 | 第 3 轮 | 变化 |
|---------|--------|--------|------|
| 提示依赖 | 80% (4/5) | {summary['category_stats'].get('提示依赖', {}).get('function_match', 0)}/{summary['category_stats'].get('提示依赖', {}).get('total', 1)} ({summary['category_stats'].get('提示依赖', {}).get('function_match', 0)/summary['category_stats'].get('提示依赖', {}).get('total', 1)*100:.0f}%) | {'⬆️ 提升' if summary['category_stats'].get('提示依赖', {}).get('function_match', 0) > 4 else '➡️ 持平' if summary['category_stats'].get('提示依赖', {}).get('function_match', 0) == 4 else '⬇️ 下降'} |
| 逃避难度 | 60% (3/5) | {summary['category_stats'].get('逃避难度', {}).get('function_match', 0)}/{summary['category_stats'].get('逃避难度', {}).get('total', 1)} ({summary['category_stats'].get('逃避难度', {}).get('function_match', 0)/summary['category_stats'].get('逃避难度', {}).get('total', 1)*100:.0f}%) | {'⬆️ 提升' if summary['category_stats'].get('逃避难度', {}).get('function_match', 0) > 3 else '➡️ 持平' if summary['category_stats'].get('逃避难度', {}).get('function_match', 0) == 3 else '⬇️ 下降'} |
| 寻求关注 | 60% (3/5) | {summary['category_stats'].get('寻求关注', {}).get('function_match', 0)}/{summary['category_stats'].get('寻求关注', {}).get('total', 1)} ({summary['category_stats'].get('寻求关注', {}).get('function_match', 0)/summary['category_stats'].get('寻求关注', {}).get('total', 1)*100:.0f}%) | {'⬆️ 提升' if summary['category_stats'].get('寻求关注', {}).get('function_match', 0) > 3 else '➡️ 持平' if summary['category_stats'].get('寻求关注', {}).get('function_match', 0) == 3 else '⬇️ 下降'} |
| 感觉逃避 | 60% (3/5) | {summary['category_stats'].get('感觉逃避', {}).get('function_match', 0)}/{summary['category_stats'].get('感觉逃避', {}).get('total', 1)} ({summary['category_stats'].get('感觉逃避', {}).get('function_match', 0)/summary['category_stats'].get('感觉逃避', {}).get('total', 1)*100:.0f}%) | {'⬆️ 提升' if summary['category_stats'].get('感觉逃避', {}).get('function_match', 0) > 3 else '➡️ 持平' if summary['category_stats'].get('感觉逃避', {}).get('function_match', 0) == 3 else '⬇️ 下降'} |

---

**报告生成时间**: {datetime.now().strftime('%Y-%m-%d %H:%M')}  
**版本**: V4.5.3-Round3  
**状态**: {'✅ 测试通过，P0 已修复' if summary['test_info']['intervention_match'] == 20 and summary['test_info']['intervention_clean'] == 20 else '⚠️ 仍需优化'}
"""
