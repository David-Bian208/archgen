      ...
      <a-spin dot :loading="loading" tip="加载中...">
            ...
            <div v-else-if="topics.length" class="topic-list">
              ...
              >
                ...
                <div v-if="t.direction_score || t.deficiency_score || t.overall_score" style="margin-top: 12px; padding-top: 12px; border-top: 1px dashed #e5e6eb">
                  ...
                  <a-row :gutter="12" style="margin-bottom: 10px">
                    <a-col :span="12">
                      <div class="score-card" :class="{ 'score-high': t.direction_score >= 80, 'score-medium': t.direction_score >= 60 && t.direction_score < 80, 'score-low': t.direction_score < 60 }">
                        <div class="score-title">🎯 方向适合度 <span style="font-weight: 400; font-size: 10px; color: #86909c">AI 判断</span></div>
                    ...
                    <a-col :span="12">
                      <div class="score-card" :class="{ 'score-high': t.overall_score >= 80, 'score-medium': t.overall_score >= 60 && t.overall_score < 80, 'score-low': t.overall_score < 60 }">
                        ...
                        <div class="score-formula" style="font-size: 10px; color: #86909c; margin-top: 4px; line-height: 1.2">
                          {{ getScoreFormula(t).replace('综合评分 = ', '') }}
                        </div>
                        <div class="score-tag">
                  ...
                  <!-- 第二行：素材雷达（全宽）-->
                  <div class="radar-fullcard" :class="{ 'radar-good': radarVerdictLevel(t) === 'good', 'radar-warn': radarVerdictLevel(t) === 'warn', 'radar-bad': radarVerdictLevel(t) === 'bad' }" @click="$event.stopPropagation()">
                    <div class="radar-header">
                      <span class="radar-title">📡 素材雷达</span>
                      <span class="radar-source-hint">系统扫描</span>
                      <span class="radar-count">
                        {{ radarGoodCount(t) }}/{{ radarTotalCount(t) }} 项就绪
                      </span>
                      <span class="radar-verdict" :class="'verdict-' + radarVerdictLevel(t)">
                        {{ radarVerdict(t) || '扫描中...' }}
                      </span>
                    </div>
                    <!-- 信号灯网格：5 项横向排列（调试：强制显示） -->
                    <div class="radar-signals" v-if="true">
                      <div v-for="(s, si) in radarSignals(t)" :key="si" class="radar-signal-item" :class="{ 'signal-ok': s.ok, 'signal-ng': !s.ok }">
                        <span class="signal-icon">{{ s.ok ? '✅' : '⚠️' }}</span>
                        <span class="signal-label">{{ s.label }}</span>
                        <span class="signal-detail">{{ s.detail }}</span>
                      </div>
                    </div>
                    <!-- 无数据回退：只有当 radarSignals 为空时才显示 -->
                    <div v-else class="completeness-detail">
                      <div v-for="(d, di) in [...deficiencyList(t), ...satisfiedList(t)]" :key="di" class="comp-detail-item" :class="{ 'item-bad': d.severity, 'item-good': !d.severity }">
                        <span class="comp-detail-title">
                          {{ d.severity === 'high' ? '🔴' : d.severity === 'medium' ? '🟡' : '✅' }} {{ d.item }}
                        </span>
                        <p class="comp-detail-text">{{ d.explanation }}</p>
                      </div>
                    </div>
                  </div>
                </div>  <!-- closes 2-row score div -->
        ...
        <div v-if="currentStep === 1" class="step-content">
          ...
          <StepSupplement
            ...
            :ai-supplement-materials="aiSupplementMaterials"
            :ai-supplement-materials-loading="aiSupplementMaterialsLoading"
        ...
        <div v-if="currentStep === 3" class="step-content">
          ...
          <a-card title="完整文章">
            <!-- 流式生成进度 -->
            <div v-if="!articleResult && articleProgress.length > 0" style="margin-bottom: 20px">
              <div v-for="(p, idx) in articleProgress" :key="idx"
                   style="padding: 8px 12px; margin-bottom: 4px; border-radius: 4px; display: flex; align-items: center; gap: 8px"
                   :style="{ background: p.status === 'failed' ? '#fff2f0' : p.status === 'done' ? '#f6ffed' : p.status === 'writing' ? '#e6f4ff' : '#f7f8fa' }">
                <span style="font-size: 14px">
                  {{ p.status === 'done' ? '✅' : p.status === 'failed' ? '❌' : p.status === 'writing' ? '✍️' : p.status === 'retrying' ? '🔄' : '⏳' }}
                </span>
                <span style="flex: 1; font-size: 13px; color: #1d2129">{{ p.title }}</span>
                <span v-if="p.status === 'failed'" style="font-size: 11px; color: #f53f3f" :title="p.error">{{ p.error.slice(0, 40) }}</span>
                <span v-if="p.status === 'retrying'" style="font-size: 11px; color: #fa8c16">重试中</span>
              </div>
              <div v-if="articlePolishing" style="padding: 8px 12px; background: #fff7e6; border-radius: 4px; text-align: center; font-size: 13px; color: #d46b08">
                🖋️ 正在润色整篇文章...
              </div>
            </div>
            <div v-if="!articleResult && articleProgress.length === 0" style="text-align: center; padding: 40px; color: #86909c">
              点击"开始生成文章"按钮...
            </div>
            ...
            <div v-else-if="articlePolishing" style="text-align: center; padding: 40px">
              <div style="font-size: 36px; margin-bottom: 12px">🖋️</div>
              <div style="font-size: 15px; color: #d46b08; font-weight: 500">正在润色整篇文章...</div>
              <div style="font-size: 12px; color: #86909c; margin-top: 8px">衔接段落、优化过渡、统一风格</div>
            </div>
            <div v-else style="text-align: center; padding: 40px">
...
import {
  ...
  getAipulseStatus,
  buildMaterialPool,
  supplementMaterials as apiSupplementMaterials,
} from '../utils/api'
...
const topicAngles = ref([])  // 题目驱动的角度
const selectedAngle = ref(null)
const angleLoading = ref(false)
let angleRequestId = 0  // 竞态保护：忽略过期请求的响应
...
// V7 流式文章生成进度
const articleProgress = ref([])  // [{ title, status: 'writing'|'done'|'failed', error: '' }]
const articlePolishing = ref(false)
let articleGenController = null  // SSE 流式文章生成 abort controller
...
watch(currentStep, (newStep) => {
  ...
  if (newStep === 2) {
    ...
    // 进入时清空，等待 AI 推荐结果，不再预先塞默认角度（避免与真实推荐混淆）
    availableAngles.value = []
    topicAngles.value = []
    // 异步加载推荐角度（耗时 ~30s，不阻塞 UI）
    loadAngles()
  }
})
...
async function loadAngles() {
  ...
  const requestId = ++angleRequestId  // 竞态保护：当前请求编号
  try {
    // 构建素材摘要
    ...
    // 同时加载素材驱动和题目驱动角度
    const [materialRes, topicRes] = await Promise.all([
      apiRecommendAngles({
    ...
    // 竞态保护：如果这不是最新的请求，丢弃本次结果
    if (requestId !== angleRequestId) {
      console.log('📐 忽略过期请求 #' + requestId + '，当前为 #' + angleRequestId)
      return
    }
    
    if (materialRes.data?.code === 0 && materialRes.data.data?.angles) {
      availableAngles.value = materialRes.data.data.angles
      console.log('📐 素材驱动角度推荐已加载:', availableAngles.value.length, '个角度')
    } else {
      // 素材推荐未返回有效数据时清空素材区，不再兜底默认角度，避免与题目推荐区的真实角度混淆
      console.warn('📐 素材驱动角度推荐未返回有效数据:', materialRes?.data?.code, materialRes?.data?.data)
      availableAngles.value = []
    }
    
    if (topicRes?.data?.code === 0 && topicRes.data.data?.angles) {
      topicAngles.value = topicRes.data.data.angles
      console.log('📐 题目驱动角度推荐已加载:', topicAngles.value.length, '个角度')
    } else {
      topicAngles.value = []
    }
  } catch (e) {
    if (requestId !== angleRequestId) return  // 过期请求静默丢弃
...
function selectAngle(angle) {
  ...
  // 构造结构化角度上下文
  const inMaterial = availableAngles.value.some(a => a.name === angle.name)
  const inTopic = topicAngles.value.some(a => a.name === angle.name)
  const angleContext = {
    angle_name: angle.name,
    angle_desc: angle.description || '',
    coverage: angle.coverage,
    gaps: angle.gaps || [],
    dimensions: angle.dimensions || [],
    source: inMaterial ? 'material' : inTopic ? 'topic' : 'custom',
  }
  ...
  console.log('🚀 选择角度后启动 startStreamSlots:', { angleTopic, summary: summary.slice(0, 200) + '...', angleContext })
  startStreamSlots(angleTopic, summary, angleContext)
}
...
async function handleEvaluateAngle(angleName, angleDescription, callback) {
  if (!angleName.trim()) return
  try {
    const res = await evaluateCustomAngle({
      sessionId: sessionId.value,
      angleName,
    ...
    if (res.data.code === 0) {
      ...
      topics.value = (res.data.data.topics || []).map(t => {
        ...
        return {
          ...
          material_score,
          evaluation: evalData,
          signal_report: t.signal_report || evalData.signal_report || {}
        }
      })
...
// 获取缺失项列表（兼容新旧后端格式）
function deficiencyList(t) {
  // 优先 topic 级别
  const dd = t.deficiency_details
  if (dd && dd.length) return dd
  // 回退到 evaluation 级别
  const edd = t.evaluation?.deficiency_details
  return edd || []
}

// 获取满足项列表（兼容新旧后端格式）
function satisfiedList(t) {
  const ss = t.satisfied_details
  if (ss && ss.length) return ss
  const ess = t.evaluation?.satisfied_details
  return ess || []
}

// ───── 素材雷达辅助函数 ─────
function radarReport(t) {
  return t.signal_report || t.evaluation?.signal_report || {}
}
function radarSignals(t) {
  const r = radarReport(t)
  return r.signal_details || []
}
function radarVerdict(t) {
  return radarReport(t).verdict || ''
}
function radarVerdictLevel(t) {
  return radarReport(t).verdict_level || 'warn'
}
function radarGoodCount(t) {
  return radarReport(t).good_count ?? 0
}
function radarTotalCount(t) {
  return radarReport(t).total_count ?? 5
}

// 获取综合评分公式文本
function getScoreFormula(t) {
  // 优先使用后端返回的 _score_formula
  if (t._score_formula) return t._score_formula
  // 前端计算
  const dir = t.direction_score || t.evaluation?.direction_score || 0
  const mat = t.material_score || t.evaluation?.material_score || 0
  const overall = t.overall_score || t.evaluation?.overall_score || 0
  return `综合评分 = 方向适合度×0.6 + 内容完整度×0.4 = ${dir}×0.6 + ${mat}×0.4 = ${overall}`
}
...
async function selectDirectionAndAdvance(t) {
  ...
  // 获取雷达匹配文档（选题 → 写作桥梁）
  try {
    const poolRes = await buildMaterialPool(
      sessionId.value,
      mcpSummary.value,
      mcpFiles.value
    )
    if (poolRes.data?.code === 0) {
      const poolData = poolRes.data.data?.material_pool || []
      // 清除旧雷达文档防止重复
      materialPool.value = materialPool.value.filter(m => m.type !== 'radar_matched')
      poolData.forEach(item => {
        if (item.source_type === 'radar_matched') {
          materialPool.value.push({
            type: 'radar_matched',
            name: item.filename || item.text?.slice(0, 50) || '',
            content: item.text || '',
            source_type: item.source_type,
          })
        }
      })
      console.log(`📡 雷达匹配文档已加载: ${materialPool.value.filter(m => m.type === 'radar_matched').length} 篇`)
    }
  } catch (e) {
    console.warn('加载雷达匹配文档失败:', e)
  }
  ...
  try {
    ...
    if (res.data.code === 0 && res.data.data) {
      preCheckResult.value = {
        status: res.data.data.status || 'initializing',
        score: res.data.data.overall_score || 0,
        score_source: res.data.data.score_source || '',
        radar_basis: res.data.data.radar_basis || null,
        writing_progress: res.data.data.writing_progress || '',
        writing_suggestions: res.data.data.writing_suggestions || [],
        issues: (res.data.data.issues || []).filter(issue => issue.type !== 'pass'),
        ready_for_next: res.data.data.ready_for_next || false,
      }
      const status = res.data.data.status || ''
      if (status === 'initializing') {
        Message.success('📡 素材库存已核验')
      } else if (status === 'pending') {
        Message.success('📦 已检测到分析草稿，选框架后可评估')
      } else {
        Message.success('📊 内容质量评估完成')
      }
    }
  } catch (e) {
...
// AI 补充素材：根据雷达缺口自动搜索知识库补充文档
const aiSupplementMaterialsLoading = ref(false)
async function aiSupplementMaterials() {
  if (aiSupplementMaterialsLoading.value) return
  aiSupplementMaterialsLoading.value = true
  try {
    const directionName = selectedDirection.value?.name || ''
    const res = await apiSupplementMaterials(sessionId.value, directionName)
    const data = res.data?.data || {}
    const { added_files = [], new_density, before_density, suggestion, diag_log = [] } = data

    // 构建诊断过程展示
    const diagContent = diag_log.length > 0 ? diag_log.join('\n') : ''

    if (added_files.length > 0) {
      // 直接将新增文档加入前端 materialPool（标记 source_type 为 ai_supplement）
      for (const f of added_files) {
        materialPool.value.push({
          ...
          _gap_addressed: f.gap,
        })
      }
      const densityChange = Math.round((new_density - before_density) * 100)
      const densityStr = densityChange > 0 ? `（密度 ${Math.round(before_density * 100)}% → ${Math.round(new_density * 100)}%）` : ''
      const summary = `✅ 补充了 ${added_files.length} 篇文档 ${densityStr}`
      
      // 诊断过程弹窗
      Modal.success({
        title: 'AI 补充素材',
        content: () => {
          return h('div', { style: 'white-space: pre-line; line-height: 1.8; font-size: 13px;' }, [
            h('div', { style: 'margin-bottom: 12px; font-weight: 600; color: #1d2129' }, summary),
            h('div', { style: 'color: #4e5969' }, diagContent),
          ])
        },
        okText: '知道了',
        simple: true,
      ...
      if (diag_log.length > 0) {
        Modal.info({
          title: 'AI 补充素材',
          content: diag_log.join('\n'),
          okText: '知道了',
          simple: true,
        })
      } else {
      ...
      Message.info('当前方向无素材缺口，无需补充')
    }
  } catch (e) {
    console.error('AI 补充素材失败:', e)
    Message.error('AI 补充素材失败: ' + e.message)
  } finally {
    aiSupplementMaterialsLoading.value = false
  }
}
...
async function generateArticleV4() {
  ...
  articleProgress.value = confirmedSlots.value.map(s => ({ title: s.label, status: 'pending' }))
  articlePolishing.value = false
  ...
  articleGenController = generateFullArticleStream(sessionId.value, sections, {
    target_word_count: targetWordCount.value,
  }, {
    onParagraphStart(data) {
      const idx = articleProgress.value.findIndex(p => p.title === data.title)
      if (idx >= 0) articleProgress.value[idx] = { ...articleProgress.value[idx], status: 'writing' }
    },
    onParagraphDone(data) {
      const idx = articleProgress.value.findIndex(p => p.title === data.title)
      if (idx >= 0) articleProgress.value[idx] = { ...articleProgress.value[idx], status: 'done' }
    },
    onParagraphRetry(data) {
      const idx = articleProgress.value.findIndex(p => p.title === data.title)
      if (idx >= 0) articleProgress.value[idx] = { ...articleProgress.value[idx], status: 'retrying', error: data.reason }
    },
    onParagraphFailed(data) {
      const idx = articleProgress.value.findIndex(p => p.title === data.title)
      if (idx >= 0) articleProgress.value[idx] = { ...articleProgress.value[idx], status: 'failed', error: data.error }
    },
    onPolishing() {
      articlePolishing.value = true
      loading.value = false  // 关掉 spinner，展示润色提示
    },
    onPolishedArticle(data) {
      // 最短展示 1.5s，确保润色提示可见
      const finished = () => {
        articleResult.value = data
        articlePolishing.value = false
        Message.success(data.generation_warnings?.length
          ? `文章生成完成（${data.generation_warnings.length} 段失败）`
          : '文章生成完成')
      }
      if (articlePolishing.value) {
...
function goToGenerateArticle() {
  if (confirmedSlots.value.length > 0) {
    generateArticleV4()
  } else if (outlineResult.value?.sections) {
    generateArticleV4FromOutline()
  } else {
    Message.warning('请先确认槽位或生成提纲')
    return
  }
}

// V4 降级版：从 outlineResult 构建 sections，走流式生成
function generateArticleV4FromOutline() {
  loading.value = true
  currentStep.value = 3
  articleResult.value = null

  // 从 outlineResult 的 sections 构建段落列表
  const outlineSections = outlineResult.value?.sections || {}
  const sectionEntries = typeof outlineSections === 'object' && !Array.isArray(outlineSections)
    ? Object.entries(outlineSections)
    : []

  // 收集 Step 5 检测页补充 + Step 6 版块素材
  const step5Supplements = supplement2Text.value || ''
  const step6Materials = []

  const sections = sectionEntries.map(([key, section]) => {
    const mats = section?.materials?.has || []
    if (mats.length > 0) {
      step6Materials.push({ section_key: key, title: section.title, materials: mats })
    }
    return {
      key,
      ...
      materials: { has: mats.map(String) },
    }
  })

  if (sections.length === 0) {
    loading.value = false
    Message.warning('提纲中无有效段落，请先生成提纲')
    return
  }

  articleProgress.value = sections.map(s => ({ title: s.title, status: 'pending' }))
  articlePolishing.value = false
...
// 保留旧版 generateArticle 作为纯函数（不再被 goToGenerateArticle 调用）
...
async function batchUnifiedSupplement() {
  ...
  for (let i = 0; i < missingItems.length; i++) {
    const item = missingItems[i]
    // 优先使用 suggested_keywords（方案 C 数据链路），回退为前 10 字符截断
    const itemObj = typeof item === 'object' ? item : null
    const keywords = itemObj?.suggested_keywords && itemObj.suggested_keywords.length
      ? itemObj.suggested_keywords
      : [typeof item === 'string' ? item.substring(0, 30) : 'AI']
    
    try {
      // Step 1: API 检索
      let apiCases = []
      try {
        const res = await apiAiPulseSupplement(sessionId.value, item, keywords)
        if (res.data.code === 0) {
...
.radar-fullcard.radar-good { border-left: 3px solid #00b42a; }
.radar-fullcard.radar-warn { border-left: 3px solid #f7ba1e; }
.radar-fullcard.radar-bad  { border-left: 3px solid #f53f3f; }
...
.radar-title { font-size: 12px; color: #86909c; }
.radar-source-hint {
  font-size: 10px;
  color: #c9cdd4;
  margin-left: 6px;
...
.radar-count { font-size: 11px; color: #1d2129; font-weight: 600; }
.radar-verdict {
  margin-left: auto;
  font-size: 12px;
  font-weight: 600;
...
.verdict-warn { background: #fff8e1; color: #e65100; }
...
@media (max-width: 768px) {
  .radar-signals { grid-template-columns: 1fr; }
}
...
.radar-signal-item.signal-ok { border-left: 2px solid #00b42a; }
.radar-signal-item.signal-ng { border-left: 2px solid #f7ba1e; }

.signal-icon { font-size: 12px; flex-shrink: 0; }
.signal-label { font-size: 12px; font-weight: 600; color: #1d2129; }
.signal-detail {
  font-size: 11px;
  color: #4e5969;
  line-height: 1.3;
...
@media (max-width: 900px) {
  .completeness-detail { grid-template-columns: repeat(2, 1fr); }
}
@media (max-width: 768px) {
  .completeness-detail { grid-template-columns: 1fr; }
}
.comp-detail-item {
  background: #fff;
  border-radius: 4px;
...
.comp-detail-item.item-bad { border-left: 2px solid #f53f3f; }
.comp-detail-item.item-good { border-left: 2px solid #00b42a; }
.comp-detail-title {
  display: block;
  font-size: 12px;
  font-weight: 600;
