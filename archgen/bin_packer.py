...
def _effective_density(card: dict) -> str:
    ...
    规则（取 LLM 标注和客观判定中更宽松的一个）：
    - items ≤ 4 且每条 ≤ 18 字 → 客观 low
    - items ≥ 5 或某条 > 30 字 → 客观 high
    - 其他 → 客观 medium
    ...
    n_items = len(items)
    max_chars = max((len(it.get("text", "")) for it in items), default=0)

    # 客观判定
    if n_items <= 4 and max_chars <= 18:
        objective = "low"
    elif n_items >= 5 or max_chars > 30:
        objective = "high"
    else:
        objective = "medium"
...
def max_slots_for(card: dict, canvas_type: str) -> int:
    ...
    # 规则1: flow/grid → 独占（视觉结构不容切割）
    if shape in ("flow", "grid"):
        return 1
    ...
    if canvas_type == "portrait":
        # 竖版：low density 的 list/compare 能双列
        if shape in ("list", "compare") and density == "low":
            return 2
        # medium 在竖版里也得独占（500px 半宽不够）
        return 1
